...
void vdmScanTreeAnalyzerDG_onData2()
{
  ...
  //TFile *f = TFile::Open("../vdm_2016Analysis/2016Scans_v3.root");


  //TFile *f = TFile::Open("../vdm_2016Analysis/2016Scans_v5.root");
...
RooRealVar yWidthN1("yWidthN1","yWidthN1",1.3,3.0) ;
 ...
 simPdf.addPdf(beam1RestVerticesUnfold_YScan,"Y_ScanData_Beam1Rest");
  ...
  RooFitResult* r= simPdf.fitTo(combData,RooFit::PrintLevel(3),RooFit::Verbose(1),RooFit::Save(),"ILL");
...
TRandom3 rand;
  rand.SetSeed(0);
 TFile *fAna = new TFile("DataAnalysisBunch"+bunchStr[i]+"DG_withML_"+suff+"_NewScale.root","recreate");
