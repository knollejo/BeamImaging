...
Double_t beamMultSupDG(Double_t *x,Double_t *par)
{
  Double_t arg = 0;
...
void vdmScanTreeAnalyzerSupDG_onData2_studyUnc_2016(TString bx)
{
...
int i = 0;
  ...
  */
  ...
  RooFormulaVar w1W("w1W","(1.0-w1MFraction)*(1.0-w1N)", RooArgSet(w1MFraction,w1N)) ;
 ...
 simPdf.addPdf(beam1RestVerticesUnfold_YScan,"Y_ScanData_Beam1Rest");
  ...
  TF2 *multBeam = new TF2("multBeam",beamMultSupDG,-30,30,-30,30,26);
...
*/
    ...
    TRandom3 rand;
    rand.SetSeed(0);
    TFile *fAna = new TFile("DataAnalysisBunch"+bunchStr[i]+"SupDG_new_"+suff+".root","recreate");
 ...
 //Errors
    ...
    weight1N_error_h->Fill(w1N.getError());
    TH1F *weight1M_error_h = new TH1F("weight1M_error_h","weight1M_error_h",200,0.0,1.);
    double wM1_Error = TMath::Sqrt(TMath::Power((1.0-w1N.getValV())*w1MFraction.getError(),2.0)+TMath::Power(w1MFraction.getValV()*w1N.getError(),2.0));
    weight1M_error_h->Fill(wM1_Error);
    TH1F *weight1W_error_h = new TH1F("weight1W_error_h","weight1W_error_h",200,0.0,1.);
    double wW1_Error = TMath::Sqrt(TMath::Power((1.0-w1N.getValV())*w1MFraction.getError(),2.0)+TMath::Power((1.0-w1MFraction.getValV())*w1N.getError(),2.0));
    weight1W_error_h->Fill(wW1_Error);
    TH1F *weight2N_error_h = new TH1F("weight2N_error_h","weight2N_error_h",200,0.0,1.);
    weight2N_error_h->Fill(w2N.getError());
    TH1F *weight2M_error_h = new TH1F("weight2M_error_h","weight2M_error_h",200,0.0,1.);
    double wM2_Error = TMath::Sqrt(TMath::Power((1.0-w2N.getValV())*w2MFraction.getError(),2.0)+TMath::Power(w2MFraction.getValV()*w2N.getError(),2.0));
    weight2M_error_h->Fill(wM2_Error);
    TH1F *weight2W_error_h = new TH1F("weight2W_error_h","weight2W_error_h",200,0.0,1.);
    double wW2_Error = TMath::Sqrt(TMath::Power((1.0-w2N.getValV())*w2MFraction.getError(),2.0)+TMath::Power((1.0-w2MFraction.getValV())*w2N.getError(),2.0));
  ...
  for(int k=0; k<1000;k++)
    {
      ...
      double neww1n = rand.Gaus(w1N.getValV(),w1N.getError());
      double newfraction1 = rand.Gaus(w1MFraction.getValV(),w1MFraction.getError());
      multBeam->SetParameter(11,neww1n);
      multBeam->SetParameter(12,newfraction1*(1.0-neww1n));
      multBeam->SetParameter(13, 0.0);
      multBeam->SetParameter(14, 0.0);
      ...
      double neww2n = rand.Gaus(w2N.getValV(),w2N.getError());
      double newfraction2 = rand.Gaus(w2MFraction.getValV(),w2MFraction.getError());
      multBeam->SetParameter(24,neww2n);
      multBeam->SetParameter(25,newfraction2*(1.0-neww2n));
